...
void renderNPC(struct NPC *npc, struct Camera *camera) {
  if (npc->state == NPC_STILL || npc->state == NPC_DONE)
    return;
...
void updateCaveState(struct Player *player, struct Background *bg,
                     struct Midground *mg, struct Camera *camera,
                     int *gameState, struct Pickup pickups[]) {
  ...
  if (eKey && !eKeyWas && npc.showPrompt && !npc.isInteracting &&
      npc.state == NPC_TURNING) {
    npc.isInteracting = 1;
    npc.dialogueIndex = 0;
  ...
  if (!player->onGround) {
    ...
    if (player->vy <= 0) {
      ...
      if (player->y <= CAVE_P3_Y && prevY >= CAVE_P3_Y &&
          player->x >= CAVE_P3_X_MIN && player->x <= CAVE_P3_X_MAX) {
        player->y = CAVE_P3_Y;
        player->vy = 0;
        player->onGround = 1;
        ...
        player->stateTimer = 0;
        playLandSound();
      } else if (player->y <= CAVE_P2_Y && prevY >= CAVE_P2_Y &&
                 player->x >= CAVE_P2_X_MIN && player->x <= CAVE_P2_X_MAX) {
        player->y = CAVE_P2_Y;
        player->vy = 0;
        player->onGround = 1;
        setPlayerState(player, LANDING);
        ...
        player->stateTimer = 0;
        playLandSound();
        if (player->x < CAVE_P1_X_MIN)
          player->x = CAVE_P1_X_MIN;
        if (player->x > CAVE_P1_X_MAX)
          player->x = CAVE_P1_X_MAX;
      }
    }
  } else {
    ...
    if (onP1) {
      ...
      if (player->x < CAVE_P3_X_MIN)
        player->x = CAVE_P3_X_MIN;
