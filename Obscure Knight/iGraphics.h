...
static void __stdcall keypressHandler(HWND, unsigned int, unsigned int,
                                      unsigned long) {
  // fixedUpdate();
}

int isKeyPressed(unsigned char key) { return keyPressed[key]; }

int isSpecialKeyPressed(unsigned char key) { return specialKeyPressed[key]; }
